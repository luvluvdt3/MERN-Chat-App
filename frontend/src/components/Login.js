...
const Login = () => {
  ...
  const login = (e) => {
    e.preventDefault();
    //console.log(state)
    dispatch(userLogin(state)) //update the state through authAction
  }

  useEffect(()=>{ //the first thing to do when the page finished loading 
    if(authenticate){
         navigate('/');
    }
    if(successMessage){ //if there is success message in the state (which modifiwd by authReducer)
